...
-u: url (url frequency bins are posted to, default http://localhost:5100/data)\n\
-v: verbose\n";

const char *sample_usage = "\
sample args:\n\
listen to localhost:1234 for 1 sensor with 2 channels of endian swapped data containing header with sample rate 10000 posting frequency bins to http://localhost:5100/data: -o localhost -p 1234 -s 1 -c 2 -e 1 -h 1 -r 10000 -b 'http://localhost:5100/data'\n";
...
int main(int argc, char **argv) {
	...
	double gradient = 1.0; // 0.0000305185;
	double offset = 0.0;
	...
	exit_failure_if(name_buffer == NULL, "failed to allocate memeory");
	...
	while ((proceed) &&
		...
		for (int i = 0, k = 0; i < pc.sensor_count; i++) {
			for (int j = 0; j < pc.channel_count; j++) {
				...
				pass_frequency_bins(v, 1, 1000, 5);
